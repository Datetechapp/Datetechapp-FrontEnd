    ...
    if (value === '') {
      if (index > 0 && inputRefs[index - 1]?.current) {
        ...
        if (prevInputRef && prevInputRef.setSelectionRange) {
          prevInputRef.setSelectionRange(
            prevInputRef.value.length,
            prevInputRef.value.length,
          );
        }
      }
      ...
      if (
        ...
        value.length >= currentInputRef.maxLength
      ) {
    ...
    <div className={css.verificationPageWrapper}>
      ...
      <div className={css.main}>
        <div className={css.formBlock}>
          ...
          <div className={css.inputsBlock}>
            {values.map((value, index) => (
              <Input
                key={index}
                className={
                  !errorMessage ? css.inputForVerify : css.inputWithError
                }
                value={value}
                type="text"
          ...
          <p
            className={
              resendText === 'Resend code' ? css.activeLink : css.passiveLink
            }
