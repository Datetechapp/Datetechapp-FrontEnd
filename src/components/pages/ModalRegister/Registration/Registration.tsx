...
export const Registration: FC = () => {
  ...
  const handleConfirmPasswordChange = (
    ...
    setConfirmPasswordValid(
      e.target.value === passwordValue && e.target.value.length > 0,
    );
  };
  ...
  const handleSubmit = (
    ...
    if (isCodeVerified) {
      ...
      !validator.isEmail(emailOrPhoneValue) &&
      !validator.isMobilePhone(emailOrPhoneValue, 'any')
    ) {
      setErrorMessage(
        'Incorrect E-mail or Phone number. Check the character set and try again',
      );
    } else {
      registration({ username: emailOrPhoneValue })
        .then((response) => {
          if (response.ok) {
            setAuthorized(true);
            setErrorMessage('');
  ...
  return (
    <div>
      <div className={css.blockToRegistration}>
        ...
        <Link to="/login" className={css.messageHaveAnAccount}>
      ...
      <form className={css.formForEmail} onSubmit={handleSubmit} noValidate>
        <div>
          <div className={css.inputBlock}>
            <EmailOrPhoneInput
              className={
                !isFocusedEmail && emailOrPhoneValue.length === 0
                  ? css.inputForEmail
                  : isFocusedEmail
                  ? css.inputForFocusedEmail
        ...
        <Button
          className={
            isValidEmail && passwordValid && confirmPasswordValid
              ? css.btnContinuePink
              : css.btnContinue
          }
          disabled={!isValidEmail || !passwordValid || !confirmPasswordValid}
        >
          Continue
        </Button>
      </form>
      <p className={css.privacyPolicyText}>
        ...
        <span className={css.privacyPolicyLink}>Privacy Policy</span>.
      </p>
    </div>
  );
};
